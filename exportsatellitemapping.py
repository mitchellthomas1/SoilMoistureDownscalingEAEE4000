...
def mappingImagesShell(geom):
    def mappingImages(startDate):
        '''
...
for j, feature in enumerate(geometries.getInfo()['features']):
    ...
    ee_list = ee.List([ee.Date(str(d)) for d in date_index.values[:-1]])
    # ee_images is a list of ee.Images
    
    def sample_rectangle(image):
        image = ee.Image(image)
        # bandNames = ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B8A', 'B9', 'B10', 'B11', 'B12', 'VV', 'VH', 'angle', 'ssm']
        sample = ee.Feature(image.sampleRectangle(region = geom,  defaultValue=nan_value))
        return sample
        
    ee_images = ee.ImageCollection(ee_list.map(mappingImagesShell(geom)))
    ee.Image(ee_images.toList(1).get(0))
    samples = ee.FeatureCollection(ee_images.map(sample_rectangle))
    
    if j ==1:
        task = ee.batch.Export.table.toDrive(collection = samples, 
                                             description = 'Region{}Samples'.format(j+1), 
                                             folder = 'MLEnvironmentGEE', 
                                             fileNamePrefix = 'Region{}Samples'.format(j+1),
                                             fileFormat = 'CSV')
        task.start()
    ...
    # print(ee_images.size().getInfo())
    # # bands = (B1, B2,B3,B4,B5,B6,B7,B8,B8A,B9,B10,B11,B12,VV,VH,Angle, SSM)
    # bandNames = ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B8A', 'B9', 'B10', 'B11', 'B12', 'VV', 'VH', 'angle', 'ssm']
    # for i in range(ee_images.size().getInfo()):
    #     print(i)
    #     image = ee.Image(ee_images.get(i))
    #     # print('image', image.getInfo())
    #     # for k in range(16):
    #     #     image.select(k)
    #     arr = gm.ee_to_numpy(image,bands = bandNames, region=geom, properties=None, default_value=nan_value)
    #     print('arr', arr)
